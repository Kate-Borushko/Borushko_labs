...
void GasTransmissionNetwork::addConnection(std::unordered_map<uint32_t, Pipe>& mP, std::unordered_map<uint32_t, Station>& mS)
{
    if ((mP.size() > 0) && (mS.size() > 1))
    {
        ...
        {
            std::cout << "Pipe is already connected";
        }
    }
    else
    {
...
std::unordered_map<uint32_t, uint32_t> GasTransmissionNetwork::createIndexOfVertices(const std::unordered_map<uint32_t, Pipe>& mP, std::unordered_map<uint32_t, Station>& mS)
{
...
std::unordered_map<uint32_t, uint32_t> GasTransmissionNetwork::createIndexOfVerticesReverse(const std::unordered_map<uint32_t, Pipe>& mP, std::unordered_map<uint32_t, Station>& mS)
{
    std::set<uint32_t> vertices;
    for (const auto& [i, p] : mP)
        if (p.canBeUsed() && mS.count(p.inStationID) && mS.count(p.outStationID))
...
std::vector<std::vector<uint32_t>> GasTransmissionNetwork::createGraph(const std::unordered_map<uint32_t, Pipe>& mP, std::unordered_map<uint32_t, Station>& mS)
{
...
std::vector<std::vector<double>> GasTransmissionNetwork::createMatrixWeights(const std::unordered_map<uint32_t, Pipe>& mP, std::unordered_map<uint32_t, Station>& mS)
{
    std::unordered_map<uint32_t, uint32_t> verticesIndex = createIndexOfVertices(mP, mS);
...
std::vector<std::vector<uint32_t>> GasTransmissionNetwork::createMatrixThroughput(const std::unordered_map<uint32_t, Pipe>& mP, std::unordered_map<uint32_t, Station>& mS)
{
    std::unordered_map<uint32_t, uint32_t> verticesIndex = createIndexOfVertices(mP, mS);
...
void GasTransmissionNetwork::depthFirstSearch(uint32_t vertices, std::vector<char>& cl, std::vector<uint32_t>& p, uint32_t& cycleStart, const std::vector<std::vector<uint32_t>>& ribs, std::vector<uint32_t>& result)
{
    cl[vertices] = 1;
...
void GasTransmissionNetwork::topologicalSort(const std::unordered_map<uint32_t, uint32_t>& verticesIndex)
{
...
void GasTransmissionNetwork::showTopologicalSort(const std::unordered_map<uint32_t, Pipe>& mP, std::unordered_map<uint32_t, Station>& mS)
{
...
void GasTransmissionNetwork::maxStream(uint32_t start, uint32_t finish, const std::unordered_map<uint32_t, uint32_t>& verticesIndexForOutput)
{
...
void GasTransmissionNetwork::findMaxStream(const std::unordered_map<uint32_t, Pipe>& mP, std::unordered_map<uint32_t, Station>& mS)
{
    ribs = createGraph(mP, mS);
    weightsMatrix = createMatrixWeights(mP, mS);
    throughputMatrix = createMatrixThroughput(mP, mS);
...
void GasTransmissionNetwork::findWay(uint32_t start, uint32_t finish, const std::unordered_map<uint32_t, uint32_t>& verticesIndexForOutput)
{
...
void GasTransmissionNetwork::showFindWay(const std::unordered_map<uint32_t, Pipe>& mP, std::unordered_map<uint32_t, Station>& mS)
{
    ribs = createGraph(mP, mS);
    weightsMatrix = createMatrixWeights(mP, mS);
    throughputMatrix = createMatrixThroughput(mP, mS);
